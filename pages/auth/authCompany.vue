	...
	<view class="page">
		<uni-forms ref="form" :modelValue="formData" :rules="rules">
			<uni-forms-item label="公司名称" name="name">
				<uni-easyinput type="text" v-model="formData.name" @input="change()" placeholder="请输入公司名称" />
			...
			<uni-forms-item label="信用代码" name="unifiedSocialCreditCode">
				<uni-easyinput type="text" maxlength="18" v-model="formData.unifiedSocialCreditCode"
					placeholder="请输入信用代码" />
			</uni-forms-item>

			<uni-forms-item label="联系电话" name="contactPhone">
				<uni-easyinput type="number" maxlength="12" v-model="formData.contactPhone" placeholder="请输入联系电话" />
			</uni-forms-item>
			...
			<uni-forms-item label="公司地址" name="address">
			...
			<uni-forms-item label="上传营业执照" name="idCardNo">
				<uni-file-picker v-model="formData.licensePic" class="picker" @select="select" limit="1"></uni-file-picker>
			</uni-forms-item>
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.data.isExamine === 1) {
							...
							this.isAuth = res.data.data.isExamine
						}
					}
				})
			},
