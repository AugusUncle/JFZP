<template>
	<view class="box">
		...
		<view class="page">
			<view v-if="btnshow0" style="font-size: 28px; font-weight: 400;margin-top: 1vh;">发布期望岗位</view>
			<view v-if="btnshow1" style="font-size: 28px; font-weight: 400;margin-top: 1vh;">修改期望岗位</view>
			<view style="font-size: 14px; color: grey; margin-top: 6px;">精确的信息有利于岗位的匹配</view>
			...
			<view style="display: flex;flex-direction: row; align-items: center; margin: 2vh 0;">
				<view style="margin: 1.5vh 0;">求职类型</view>
				<view class=" ">
					<uni-segmented-control :current="data.jobType" :values="items" :style-type="styleType"
			...
			<view style="border-bottom: 0.1px solid #f7f7f7; margin-bottom: 2vh;"></view>
			<view>
				<view style="font-size: 14px; color: gray;">期望岗位</view>
				<view style="display: flex; flex-direction: row;align-items: center; margin: 2vh 0;">
					<view class="section">
						<!-- <view class="cityT">所在城市</view> -->
						<uni-data-picker :placeholder="info" style="width: 100vw;" :clear-icon="false"
							popup-title="请选择期望岗位" :localdata="jobs"  @change="onchange1()">
						</uni-data-picker>
					</view>
			...
			<view style="border-bottom: 0.1px solid #f7f7f7; margin-bottom: 2vh;"></view>
			<view>
				<view style="font-size: 14px; color: gray;">期望薪资</view>
				<view style="display: flex; flex-direction: row;align-items: center; margin: 2vh 0;">
					<view style="display: flex; flex-direction: row; align-items: center; margin: 2vh 0;">
						<picker mode="multiSelector" @change="bindPickerChange1" style="width: 100vw; " :value="index1"
							:range="[arr1,arr2]">
							<view v-if="type==='0'" class="uni-input" style="margin-left: 4px;">{{arr1[index1[0]]}}-{{arr2[index1[1]]}}K
							</view>
							<view v-if="type==='1'" class="uni-input" style="margin-left: 4px;">{{arr1[index1[0]]-1}}-{{arr2[index1[1]]-1}}K
							</view>
						</picker>
					</view>
					<uni-icons type="right" size="16" color="grey" style=" position: absolute; right: 5%; "></uni-icons>
	...
	export default {
		...
		methods: {
			...
			getJobNeed(id) {
				console.log(id)
				uni.request({
					url: this.reqUrl + '/job_expectation/' + id,
					method: 'GET',
					header: {
			...
			add() {
				...
				uni.request({
					url: this.reqUrl + '/job_expectation/save',
					method: 'POST',
					header: {
						"Authorization": this.token
					},
			...
			bindPickerChange: function(e) {
				...
				this.data.cityCode = this.provinces[this.index].id
			},
			...
			del() {
				uni.request({
					url: this.reqUrl + '/job_expectation/' + this.data.id,
					method: 'DELETE',
					header: {
						"Authorization": this.token
					},
